...
namespace FAST.Core.DataAccess
{
    ...
    public class DAAccess
    {
        ...
        public static SqlConnection _oConnection = null;
        public static SqlTransaction _oTransaciton = null;
        /// <summary>
        ...
        public DAAccess()
        {
            try
            {
                ...
                try
                {
                    ...
                    string[] sTemps = oValues["ConnectionString2"].Split(';');
                    //string sPass = Decrypt(sTemps[2]);
                    CONNECTION_STRING = sTemps[0] + ";" + sTemps[1] + ";" + sTemps[2] + ";" + sTemps[3];
                }
                catch
                {
        ...
        private SqlCommand PrepareCommand(string sSQL)
        {
            ...
            {
                throw new Exception(ex.Message + "Prepare Command");
            }
            try
            {
            ...
            {
                throw new Exception(exp.Message + " Prepare command check state");
            }
        ...
        protected int ExecuteNonQuery(string sSQL)
        {
            ...
            //         //SqlConnection.ClearPool(_oConnection);
            //         _oConnection.Close();
            // }
            oCommand.Dispose();
            return nReturn;
        ...
        #region GeneratePrimaryKey
        protected int GeneratePrimaryKey(string sTable, string sColumn)
        {
            object o;
            int nID;
            o = this.ExecuteScalar(SQL.MakeSQL("SElECT MAX(%q) FROM %q", sColumn, sTable));
            if (o == DBNull.Value)
            {
                nID = 0;
            }
