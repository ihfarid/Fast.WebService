...
namespace FAST.DataLogic
{
	public partial class DLDoctorUpdateRequestLogForRM: DAAccess
	{
		...
		public void Delete(int nDoctorUpdateRequestLogForRMID)
		{
			string sSQL = "";
			try
			{
				sSQL = SQL.MakeSQL("DELETE FROM [DoctorUpdateRequestLogForRM] WHERE [DoctorUpdateRequestID]=%n"
				, nDoctorUpdateRequestLogForRMID);
				ExecuteNonQuery(sSQL);
			}
			catch (Exception e)
			{
		...
		public IDataReader GetDoctorUpdateRequestLogForRM(int nID)
		{
			string sSQL = "";
			IDataReader oReader;
			try
			{
				sSQL = SQL.MakeSQL("SELECT * FROM [DoctorUpdateRequestLogForRM] WHERE DoctorUpdateRequestID=%n", nID);
		...
		public IDataReader GetDoctorUpdateRequestLogForRMs()
		{
			string sSQL = "";
			IDataReader oReader;
			try
			{
				sSQL = SQL.MakeSQL("SELECT * FROM [DoctorUpdateRequestLogForRM] ");
				oReader = ExecuteReader(sSQL);
			}
			catch (Exception e)
			{
        ...
        public int GetDoctorUpdateRequestID(SqlConnection oSqlConnection, SqlTransaction oSqlTransaction)
        {
            ...
            try
            {
                ...
                sSQL = SQL.MakeSQL("SElECT MAX(DoctorUpdateRequestID)+ 1 PvpID FROM DoctorUpdateRequestLogForRM");
        ...
        public int Insert(DoctorUpdateRequestLogForRM oItem, SqlConnection oSqlConnection, SqlTransaction oSqlTransaction)
        {
            string sSQL = "";
            try
            {
                ...
                sSQL = SQL.MakeSQL("INSERT INTO [DoctorUpdateRequestLogForRM](DoctorUpdateRequestID, DoctorID, TerritoryID, DoctorTypeID, Code, SwajanStatus, BMDCCode, DocName, SalutationID, SpecialtyID1, SpecialtyID2, DegreeID1, DegreeID2, Institute, Address1, Address2, Address3, DistrictID, UpazillaID, BirthDay, Mrgday, UpdateStatus, MobileNo, Email, MapAddress, MapSpeciality, MapDegree, Product1, Product2, Product3, Product4, Product5, Product6, Product7, Product8, Profile, Session, Route, CallFrequency,PostStepChange, CardAttachement, Action, Version) "
                + " VALUES(%n, %n, %s, %n, %s, %n, %s, %s, %n, %n, %n, %n, %n, %s, %s, %s, %s, %n, %n, %d, %d, %n, %s, %s, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %n, %s, %n, %n) "
                , oItem.DoctorUpdateRequestID, oItem.DoctorID, oItem.TerritoryID, oItem.DoctorTypeID, oItem.Code, oItem.SwajanStatus, oItem.BMDCCode, oItem.DocName, oItem.SalutationID, oItem.SpecialtyID1, oItem.SpecialtyID2, oItem.DegreeID1, oItem.DegreeID2, oItem.Institute, oItem.Address1, oItem.Address2, oItem.Address3, oItem.DistrictID, oItem.UpazillaID, oItem.BirthDay, oItem.Mrgday, oItem.UpdateStatus, oItem.MobileNo, oItem.Email, oItem.MapAddress, oItem.MapSpeciality, oItem.MapDegree, oItem.Product1, oItem.Product2, oItem.Product3, oItem.Product4, oItem.Product5, oItem.Product6, oItem.Product7, oItem.Product8, oItem.Profile, oItem.Session, oItem.Route, oItem.CallFrequency, oItem.PostStepChange, oItem.CardAttachement, oItem.Action, oItem.Version);
        ...
        public int Update(DoctorUpdateRequestLogForRM oItem, SqlConnection oSqlConnection, SqlTransaction oSqlTransaction)
        {
            string sSQL = "";
            try
            {
